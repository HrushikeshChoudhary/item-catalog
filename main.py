...
# Read Google client_secrets.json
# And assign client_id to a variable for later use.
CLIENT_ID = json.loads(open('client_secrets.json', 'r').read())[
    'web']['client_id']

# Connect to database and create database session.
engine = create_engine('sqlite:///catalog_database2.db',
...
def gconnect():
    ...
    output += '" 300px;border-radius: 150px;"'
...
def gdisconnect():
    ...
    atoken = login_session['access_token']
    url = 'https://accounts.google.com/o/oauth2/revoke?token=%s' % atoken
    h = httplib2.Http()
...
# API endpoints for arbitrary item
@app.route('/<category_name>/<item_name>/info.json')
def itemJSON(category_name, item_name):
    category = session.query(Category).filter_by(name=category_name).first()
    items = session.query(Item).filter_by(
        name=item_name, category=category).all()
...
def editCategory(category_name):
    ...
    if categoryToEdit.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               ...
               "your own item in order to edit.');}</script>" \
...
def deleteCategory(category_name):
    ...
    if categoryToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               " authorized to delete this item. Please create " \
               "your own item in order to delete.');}</script>" \
               "<body onload='myFunction()'>"
...
def showCategoryItems(category_name):
    ...
    usr = 'username'
    if usr not in login_session or creator.id != login_session['user_id']:
...
def editItem(category_name, item_name):
    ...
    editingItem = session.query(Item).filter_by(
        name=item_name, category=editingItemCategory).first()
...
# Edit item image
@app.route('/catalog/<category_name>/<item_name>/editimage',
           methods=['GET', 'POST'])
@login_required
...
# Delete item image
@app.route('/catalog/<category_name>/<item_name>/deleteimage',
           methods=['GET', 'POST'])
@login_required
def deleteImage(category_name, item_name):
    ...
    """Prevent current user to delete item img which belongs to other user"""
    if editingItem.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               " authorized to delete this item. Please create " \
               "your own item in order to delete.');}</script>" \
    ...
    if request.method == 'POST':
        ...
        edName = editingItem.name
        return redirect(url_for(show, category_name=catName, item_name=edName))
    else:
